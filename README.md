...
1. Download the plugin and its dependencies.
2. Put the `.jar` files into your server `plugins/` folder.
3. If the plugin includes an `arena.zip`, extract it into your **main server folder**.
...
1. Import the world (example world name: `arena`):
...
* Example:
  * `/arena create arena1`
* Set the arena waiting lobby (while inside the arena world):
  * `/arena setlobby`
    {% endstep %}
